...
def predict_Jiefeng(im, net, noiseModel, device, outScaling = 10):
    '''
    Process an image using the network, and output 800 images.
    ...
    ----------
    means: numpy array
        Image containing the 800 means of the predicted prior distribution.
    '''
    stdTorch=torch.Tensor(np.array(net.std)).to(device)
    meanTorch=torch.Tensor(np.array(net.mean)).to(device)
    ...
    #select out the small value
    samples = samples.squeeze(1) 
    samples = samples.cpu().detach().numpy()
    lines = samples[:, 48, :]   # shape: (800, 96)
    threshold = 0#这是控制最小值的,那个曲线的最小值.control the minimun value of the curve #0.4
    mask = np.max(lines[:, 44:52], axis=1) > threshold#这是控制最小值的,那个曲线的最小值.control the minimun value of the curve
    samples = samples[mask]  # shape: (N, 96, 96)
    samples = torch.from_numpy(samples).unsqueeze(1).to('cuda') 
